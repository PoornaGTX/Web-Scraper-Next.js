      ...
      {products?.length > 0 ? (
        <div className="space-y-4">
          {products?.map((product: any, index: number) =>
            product?.title ? (
              <div key={index} className="border-4 border-neutral-200 bg-white p-5 rounded-lg">
                ...
                <p className="text-md font-medium text-gray-500">{product?.price}</p>
                <p className="text-sm text-gray-700 mt-2">{product?.description}</p>
                <div className="mt-3">
                  <div className="flex flex-row w-full gap-2 flex-wrap">
                    {product?.features?.map((feature: any, featureIndex: number) => (
                      <span className="p-2 text-sm bg-blue-700 rounded-lg text-white inline-block" key={featureIndex}>
                        {feature}
                      </span>
                    ))}
                  </div>
